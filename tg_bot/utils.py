...
def wait_for_inlinekeyboard_callback(
    ...
    while True:
        ...
        if (update.callback_query.data == "skip") and cancellable:
            update.callback_query.answer()
            return update.callback_query
        if (update.callback_query.data == "cmd_done") and cancellable:
            update.callback_query.answer()
            return update.callback_query
...
def get_product_data(prods: List[dict]):
    ...
    if len(prods) == 1:
        prod = prods[0]["fields"]
        data[prod["product_id"]] = prod
        inner.append(
            InlineKeyboardButton(prod["title"], callback_data=prod["product_id"])
        )
        ...
        for prod in range(0, len(prods), 2):
            ...
            for pr in pl:
                pr = pr["fields"]
                data[pr["product_id"]] = pr
                butt = [
                    InlineKeyboardButton(pr["title"], callback_data=f"{pr['product_id']}")
                ]
                inner.extend(butt)
            keyboard.append(inner)
            inner = []
...
def buildmenubutton(data: dict, cancellable=True):
    buttons = []
    keys = list(data.keys())
